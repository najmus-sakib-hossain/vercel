  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          "absolute bottom-4 left-1/2 flex h-14 -translate-x-1/2 items-end gap-3 rounded-xl bg-opacity-90  p-2",
...
function DockCardInner({ src, id, children }: DockCardInnerProps) {
  ...
  return (
    <span className="relative z-0 flex h-full w-full items-center justify-center overflow-hidden rounded-md">
      {/* Background image with a blur effect to give a depth illusion */}
      <motion.img
        className="absolute z-10 translate-y-2.5 scale-125 opacity-40 blur-md"
        src={src}
        alt=""
      />
  ...
  return (
    <div className="flex flex-col items-center gap-1" key={id}>
      ...
      <motion.button
        ...
        className="saturate-90 transition-filter hover:brightness-112 aspect-square  rounded-lg border border-black/5 border-opacity-10 bg-neutral-100 brightness-90 duration-200 hover:saturate-100 dark:border-white/5 dark:bg-neutral-800"
